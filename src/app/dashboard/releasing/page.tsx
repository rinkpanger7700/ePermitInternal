...
import { Search, Filter, MoreVertical, FileOutput } from "lucide-react";

export default function ReleasingModule() {
  ...
  useEffect(() => {
    async function load() {
      const { data, error } = await supabase
        ...
        .select("*")
        .in("status", ["Ready for Release", "Releasing", "Released"])
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 min-h-[60vh]">
      <div className="p-6 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-800">Certificate Releasing</h2>
          <p className="text-sm text-gray-500 mt-1">Approved permits and certificates ready for release</p>
        </div>
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-gray-400">No applications ready for release.</td>
              </tr>
            ) : (
              filtered.map(app => (
                <tr key={app.reference_no} className="hover:bg-blue-50/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="inline-block px-2.5 py-1 bg-green-50 text-green-700 border border-green-200 rounded text-xs font-semibold">
                      {app.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-center gap-2">
                      <button 
                        onClick={() => router.push(`/dashboard/releasing/${encodeURIComponent(app.reference_no)}`)}
                        className="px-3 py-1.5 border border-blue-600 text-blue-600 rounded-lg text-xs font-medium hover:bg-blue-50 transition-colors flex items-center gap-1 whitespace-nowrap"
                      >
                        <FileOutput size={14} /> Release Action
                      </button>
                    </div>
                  </td>
                </tr>
