  ...
  useEffect(() => {
    async function load() {
      const { data, error } = await supabase
        ...
        .select("*")
        .in("status", ["For Inspection", "Inspection", "Inspection in Progress", "Scheduled", "Pending Assignment"])
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 min-h-[60vh]">
      <div className="p-6 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Inspection schedules, reports, and assignments</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button className="flex items-center gap-2 px-4 py-2 border border-blue-600 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-gray-400">No applications found for inspection.</td>
              </tr>
            ) : (
              filtered.map(app => (
                <tr key={app.reference_no} className="hover:bg-blue-50/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="inline-block px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded text-xs font-semibold">
                      {app.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        onClick={() => router.push(`/dashboard/inspections/${encodeURIComponent(app.reference_no)}`)}
