  ...
  useEffect(() => {
    async function load() {
      const { data, error } = await supabase
        ...
        .select("*")
        .in("status", ["For Approval", "Approval"])
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 min-h-[60vh]">
      <div className="p-6 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Board resolutions, final evaluations, and director approvals</p>
        </div>
      ...
      {!loading && filtered.length > 0 && (
        <div className="px-6 py-4 border-b border-gray-100 bg-yellow-50/50">
          <div className="flex items-center gap-2 text-yellow-800 text-sm">
            ...
            <span className="font-medium">You have {filtered.length} items pending your approval.</span>
          </div>
        </div>
      )}
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-gray-400">No applications pending approval.</td>
              </tr>
            ) : (
              filtered.map(app => (
                <tr key={app.reference_no} className="hover:bg-blue-50/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="inline-block px-2.5 py-1 bg-red-50 text-red-700 border border-red-200 rounded text-xs font-semibold">
                      {app.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-center gap-2">
                      <button 
                        onClick={() => router.push(`/dashboard/approvals/${encodeURIComponent(app.reference_no)}`)}
